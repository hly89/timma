...
#' @examples 
#' data(davis)
#' profile<-BinarizeData(davis, method="drug-specific", threshold="50fold")
#'
  ...
  if(method=="universal"){
    ...
    if(threshold=="10fold"){
      for(i in seq_len(nrow(profile))){
        ...
        # threshold
        cut.off <- min.kd*10
        if(cut.off>10000) cut.off <- 10000
        ones <- which(profile[i,]<=cut.off, arr.ind=TRUE)
        zeros <- which(profile[i,]>cut.off, arr.ind=TRUE)
        profile[i,ones] <- 1
        profile[i, zeros] <- 0
      }
